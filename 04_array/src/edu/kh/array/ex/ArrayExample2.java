...
public class ArrayExample2 {
	...
	public void deepCopy() {
		...
		// 1. for문을 이용한 깊은 복사
		int[] copyArr1 = new int[arr.length]; // 5칸 크기의 배열
		
		for(int i=0; i<arr.length; i++) {
			copyArr1[i]=arr[i];
		}
		System.out.println("\ncopyArr1 주소 : "+copyArr1);
		System.out.println("copyArr1 : "+Arrays.toString(copyArr1));
		
		// 2. System.arraycopy(A, B, C, D)
		// A : 원본배열명
		// B : 원본배열복 사 시작 인덱스
		// C : 복사배열병
		// D : 복사배열 삽입 시작 인덱스, 복사길이
		int[] copyArr2 = new int[arr.length];
		System.arraycopy(arr, 0, copyArr2, 0, arr.length);
		
		System.out.println("\ncopyArr2 주소 : "+copyArr2);
	...
	public void createLottoNumber() {
		...
		// 1. 1~45 사이의 중복되지 않은 난수 6개 생성
		// 2. 생성된 난수가 오름차순 정렬
		// 3. ex) [5,11,13,18,24,25]
		...
		for(int i=0; i<lotto.length; i++) {
			...
			for(int x=0; x<i; x++) {
				...
				if(random==lotto[x]) {
					...
					// -> i 값을 인위적으로 1 감소시켜서 난수 발생의 기회를 한번 더 주는
					break;
					// 앞쪽에서 중복데이터를 발견하면
					// 남은 값은 비교할 필요가 없다.

...
public class ServerService {
	...
	public void serverStart() {
		...
		// 클라이언트가 서버에 요청을 보내면, 서버는 클라이언트의 소켓을 받게 되므로, 
		// 서버에 클라이언트 저장용 변수가 필요하다.
		...
		try {
			...
			// -> 클라이언트가 서버 요청 시에 소켓을 전달
			// -> 소켓에 입출력 스트림 객체가 포함되어있음
			is = clientSocket.getInputStream();
			os = clientSocket.getOutputStream();
			
			// 6. 보조 스트림을 통해 성능 개선
			br = new BufferedReader(new InputStreamReader(is));
			// InputStreamReader : 문자 기반 보조 스트림
			// 문자 기반인 BufferedReader와 바이트 기반인 InputStream을 연결할 수 있도록
			...
			// getInetAddress : 클라이언트에 대핸 Ip 주소 정보를 포함하는 InetAddress 객체를 반환
			// getHostAddress : InetAddress 객체에서 IP 주소 부분을 문자열 형식으로 반환

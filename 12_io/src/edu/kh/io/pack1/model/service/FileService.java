...
public class FileService {
	...
	public void method2() {
		...
		// 문자열에서 '\' 표기하는 방법 : '\\'
		// -> 역슬래시는 이스케이프 문자로 사용됨
		// -> 역슬래시 자체를 문자로 사용하고 싶을 떈 2개를 사용해야 함
		...
		if(!file.exists()) { // 존재하지 않으면
			...
			try {
				...
				// -> 에외처리 필수
				if(file.createNewFile()) { // 반환값 : boolean // 파일이 정상 생성된 경우
	...
	public void method3() {
		...
		for(File f : files) {
			...
			// System.out.println(lastModified); // ms단위로 출력 ex) 1741135062836
			
			// java.text.simpleDateFormat : 간단히 날짜 형식을 지정할 수 있는 객체
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd a h:mm"); // ex) 2025-03-05 오전 10:33	
			
			// String SimpleDateFormat.format(long) : 
			// 매개변수 long 값을 지정된 패턴 형식으로 변환하여 문자열 형태로 변환
			
			String date = sdf.format(lastModified);
			// System.out.println(date); // ex) 2025-03-05 오전 9:37
			...
			if(f.isDirectory()) size = ""; // 폴더라면 size를 빈 문자열로 변경
				// -> 폴더 크기를 구하는 메서드는 따로 존재하지 않음
				// -> 구하려면 폴더안의 모든 파일을 순회하여 각 파일 크기를 합산해서 써야함.
			
			String result = String.format("%-15s %-25s %-5s %10S", fileName, date, type, size);
